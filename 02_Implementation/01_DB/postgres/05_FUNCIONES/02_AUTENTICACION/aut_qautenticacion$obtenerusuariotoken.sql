-- FUNCTION: ow_base_us."aut_qautenticacion$obtenerusuariotoken"(text, text, text)

CREATE OR REPLACE FUNCTION ow_base_us."aut_qautenticacion$obtenerusuariotoken"(
	p_prcs text,
	p_usuario text,
	p_token text,
...
DECLARE
    ...
    SELECT
        ...
        FROM ow_base_us.aut_tusuario, ow_base_us.aut_ttoken, ow_base_us.aut_tpersn

        WHERE usuario_usuario = COALESCE(p_usuario, usuario_usuario) AND usuario_persn = persn_persn AND usuario_tipo_aut = 'T' AND usuario_estado = 'ACT' AND token_usuario = usuario_usuario AND token_estado = 'ACT' AND token_fecexp >= CURRENT_TIMESTAMP AND token_codigo = p_token
...
ALTER FUNCTION ow_base_us."aut_qautenticacion$obtenerusuariotoken"(text, text, text)
    OWNER TO ow_base_us;
