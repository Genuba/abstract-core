-- FUNCTION: ow_base_us."aut_qusuarios$obtenerusuario"(text, text, text)

CREATE OR REPLACE FUNCTION ow_base_us."aut_qusuarios$obtenerusuario"(
	p_prcs text,
	p_usuario text,
	p_usua_clave text,
...
DECLARE
    ...
    SELECT
        ...
        WHERE usuario_usuario = COALESCE(p_usuario, usuario_usuario) AND usuario_clave = COALESCE(p_usua_clave, usuario_clave) AND usuario_estado = 'ACT' AND usuario_persn = persn_persn
...
ALTER FUNCTION ow_base_us."aut_qusuarios$obtenerusuario"(text, text, text)
    OWNER TO ow_base_us;
