-- FUNCTION: ow_base_us."ge_qmenu$serviciostplistadoporusuario"(text, text)

CREATE OR REPLACE FUNCTION ow_base_us."ge_qmenu$serviciostplistadoporusuario"(
	p_prcs text,
	p_usuario text,
...
DECLARE
    ...
    SELECT DISTINCT
        ...
        WHERE urol_usuario = p_usuario AND urol_rol = srol_rol AND servicio_servicio = srol_servicio AND surl_servicio = servicio_servicio AND surl_tipo = 'P' AND modulo_modulo = servicio_modulo AND srol_visible = 'S' AND servicio_tipo = 'L' AND servicio_servicio = mser_servicio AND menu_menu = mser_menu
...
ALTER FUNCTION ow_base_us."ge_qmenu$serviciostplistadoporusuario"(text, text)
    OWNER TO ow_base_us;
