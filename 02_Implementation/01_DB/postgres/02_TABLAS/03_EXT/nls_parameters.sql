-- Table: ow_base_ext.nls_parameters

CREATE TABLE ow_base_ext.nls_parameters
(
...
ALTER TABLE ow_base_ext.nls_parameters
    OWNER to ow_base_us;
COMMENT ON TABLE ow_base_ext.nls_parameters
    IS 'Dictionary to store NLS related params transfered from Oracle';

COMMENT ON COLUMN ow_base_ext.nls_parameters.nls_territory
    IS 'NLS_TERRITORY from Oracle';

COMMENT ON COLUMN ow_base_ext.nls_parameters.pg_territory_ux
    IS 'NLS_TERRITORY stored as part of Unix Locale';

COMMENT ON COLUMN ow_base_ext.nls_parameters.pg_territory_win
    IS 'NLS_TERRITORY stored as part of Window Locale';

COMMENT ON COLUMN ow_base_ext.nls_parameters.params_dictionary
    IS 'Nested JSON table to store actual parameters values';

-- Index: aoe_nls_territory_ux_idx

-- DROP INDEX ow_base_ext.aoe_nls_territory_ux_idx;

CREATE INDEX aoe_nls_territory_ux_idx
    ON ow_base_ext.nls_parameters USING btree
    (pg_territory_ux COLLATE pg_catalog."default")
    TABLESPACE pg_default;

-- Index: aoe_nls_territory_win_idx

-- DROP INDEX ow_base_ext.aoe_nls_territory_win_idx;
